...
import { deletedMember } from "../Api/Handellapi";
import useTask from "../hook/useTask";
import { AiFillBulb } from "react-icons/ai";

export const CardMember = ({ props }) => {
  const { memberList } = useMember();
  const { newMemberList, setNewMemberList } = props;
  ...
  const showcart = (item) => {
    ...
    taskList?.filter((task) => {
      if (task.userName === item.name) {
        tasks.push(task);
      }
    });
    const ondelete = async (id) => {
      const data = await deletedMember(item._id);
      console.log(item._id);
      deleteMember(item._id);
    };
    return (
      <div className="alluser divcss" id="them">
        <div className="divimage">
          ...
          <AiFillBulb
            className="lamp"
            style={tasks.length >= 3 ? { color: "red" } : { display: "none" }}
          />
        </div>
          ...
          <div className="divinformation">
            ...
            <p className="tasks">
              ...
              {/* {taskList?.map((task, index) => {
                return (
                  <span key={index} value={task.title}>
                    {task.title}
                  </span>
                );
              })} */}
              {/* return task.memberList.includes(name)
                  ? taskes.push({ title: task.title })
                  : ""; */}
            </p>
          </div>
          <div className="divinformation">
            ...
            <p> github address: {item.github} </p>
          </div>
        </div>
        <div className="divbutton">
          <EditMember item={item} />
  ...
  return <>{newMemberList.map(showcart)}</>;
};
